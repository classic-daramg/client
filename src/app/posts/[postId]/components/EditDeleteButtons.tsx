...
export default function EditDeleteButtons({
  ...
  // 수정 페이지로 이동
  const handleEdit = () => {
    // 포스트 타입에 따라 적절한 쿼리 파라미터와 함께 글쓰기 페이지로 이동
    let queryParams = '';
    
    if (postType === 'CURATION') {
      queryParams = '?type=curation';
    } else if (postType === 'FREE') {
      queryParams = '?type=free';
    }
    
    // TODO: 실제 수정 기능 구현 시 포스트 데이터를 draft로 저장하고 이동
    router.push(`/write${queryParams}&edit=${postId}`);
  };
      ...
      {showMenu && (
        <>
          ...
          <div className="absolute right-0 top-8 z-50 bg-white rounded-lg shadow-lg border border-[#e5e7eb] min-w-[120px] overflow-hidden">
            ...
            >
              <Image src="/icons/write.svg" alt="수정" width={16} height={16} />
              <span>수정하기</span>
            </button>
            ...
            >
              <Image src="/icons/close-white.svg" alt="삭제" width={16} height={16} className="filter-red" />
